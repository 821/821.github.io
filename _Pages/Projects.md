...
目前能檢索的辭書有大漢和辭典（部分淸晰度不佳）、中文大辭典（無詞，僅字頭）、漢語大詞典（淸晰度太高導致文件過大，無法打包使用）、漢語大字典、故訓匯纂、康熙字典、說文解字注、古文字詁林、經籍䉵詁、王力古漢語字典（早就用上了但一直懶得修正目前存在的錯誤）等。
...
Syncretext 是 Syncretismus 和 Syncrepedia 的延伸，製作對象主要是各種工具書。
...
CCL 非常不友好（不能直接查看較長的上下文），古代漢語太少且代表性太弱（現代+古代纔 1.06GB ，而且古文的主體是佛經這種重複率很高又是譯作的文本，衆所周知，凡是翻譯的文本一般都比較彆扭）。本人沒有做太多的整理，而僅僅是簡單的做了轉換成 unicode 文本的工作，所以 CCCC 可以看作是語料庫的備用庫。
...
## 尺帛 (Note-)
尺帛，言之少也。
現有筆記軟件大多沈重遲緩，食之無味的功能不可勝數，卻不能便捷的用自己習慣的文本編輯器來編輯，一個簡單的代碼高亮卻還要大書特書。龐大遲緩之餘，竟還無法在保持文件更新進度的前提下直接利用用戶散落在硬盤每個角落的零碎筆記。其實，用輕量級標記語言來寫，就能實現絕大多數筆記軟件提供的編輯效果，就差個瀏覽窗口。我就用 Python 寫一百多行代碼，配合一些支持命令行的外部工具實現所需。尺帛做的是減法，減到衹做重要的事，而且把這些事託付給業內行家去完成，自己衹是個中樞。
簡而言之，尺帛處理的是「精修細改」而非「大量收藏」的筆記，是「長篇大論」而非「隻言片語」的筆記，處理方式是「反中央化」而非「中央化」，而用戶也是「geek」向而非「小白」向。

### 特色解析
一、如果不需要 FTP 和 WebDAV ，尺帛支持 Windows, MAC, Linux 三個主流平臺。
二、尺帛可使用 Pandoc 將 Markdown, reST, Textile, LaTeX, org 等格式源文件轉爲 HTML 文件並預覽。書寫時可以不用太在意實際排版，而排版則是一次性解決的問題。
三、尺帛可通過 FTP, WebDAV, Dropbox, SimpleNote, 本地打包等方式備份。 FTP, WebDAV, 本地打包 是通用方式， Dropbox 是網盤界老大， SimpleNote 是輕便的筆記軟件且支持版本控制（所以尺帛也可看作一個支持 Markdown 等格式的跨平臺 SimpleNote 客戶端）。
四、尺帛可指使常用編輯器打開源文件。編輯器是引戰領域，故採「用戶自決原則」。

### 使用
一、下載 Note-.py 和 conf.py 。
二、安裝 Python 環境、模塊，裝好 Pandoc, WinSCP 。
三、根據自身情況塡寫 conf.py 。
四、打開 Note-.py 。
