...
## 攞你命三千 (Syncretismus)
本項目開工於 2013 年 8 月。
Syncretismus 是個掃描版辭書聯合檢索項目，目的在於同時檢索多款重要辭書，以便確認自己用詞用字的準確性。辭書源文件均爲掃描版，不存在光學識別錯誤。檢索框架選定了 GoldenDict 配合 dsl 格式，因爲能夠同時檢索多款辭書，而且 dsl 是純文本，方便後期修改、編輯。
目前能檢索的辭書有大漢和辭典（部分淸晰度不佳）、中文大辭典（無詞，僅字頭）、漢語大詞典（淸晰度太高導致文件過大，無法打包使用）、漢語大字典、故訓匯纂、康熙字典、說文解字注、古文字詁林、經籍䉵詁、王力古漢語字典（早就用上了但一直懶得修正目前存在的錯誤）、集韻、廣韻等。
...
項目地址： https://github.com/Syncretismus/Syncretismus

## 攞你命三百 (Syncrepedia)
本項目開工於 2013 年 10 月。
Syncrepedia 是 Syncretismus 的延伸，不過做的是百科。
可用辭書：中國歷史地名大辭典（史爲樂）、中國歷代官制大辭典（呂宗力）、中國歷代人名大辭典（張撝之）、四庫大辭典（李學勤）、中國道敎大辭典、中華道敎大辭典、中醫大辭典（李經緯 1995 ）、中國哲學辭典（韋政通）。
Syncrepedia 由於跨越了多門學科，辭書間關聯性不強，其實更有「攞你命三千」式的山寨感。
項目地址： https://github.com/Syncretismus/Syncrepedia

## 攞你命三億 (Syncretext)
本項目開工於 2015 年 7 月 25 日。
Syncretext 是 Syncretismus 和 Syncrepedia 的延伸。由於 Syncretismus 和 Syncrepedia 的做法是「文本索引—圖像查看」的模式，不能全文檢索和直接複製，而且直排辭書往往不好讀，所以希望能找到對應的文本數據。由於有原書可供參照， Syncretext 並不會讓人陷入文本化錯誤的泥沼，衹是方便而已。
...
CCL 非常不友好（不能直接查看較長的上下文），古代漢語太少且代表性太弱（現代+古代纔 1.06GB ，而且古文的主體是佛經這種重複率很高又是譯作的文本，且衆所周知，翻譯的文本一般都比較彆扭）。本人沒有做太多整理，而僅僅是簡單的做了轉換成 unicode 文本的工作，所以 CCCC 可以看作是語料庫的備用庫。
...
我個人對明清刻本的字體沒什麼好印象，但宋體字又是最常用的。素材考慮用中文大辭典、大漢和辭典和 Sony Reader Ming 。
（籌備中，做完臺館體就開始做他。）
...
項目地址： https://github.com/821/NoteMinus

## 寸箋 (BibInT)
寸箋，名片也。
現有文獻管理軟件均肥碩笨重，有一堆詭異的功能，卻往往沒把最基礎的問題妥善解決：文獻的輸入與輸出。所謂輸入，就是把文獻各域輸入到電腦，這一過程往往帶有大量的複製粘貼，所以一定要有相應按鈕纔能凸顯便利，但絕大多數軟件並不具備。所謂輸出，就是將文獻插入到論文——這個部分的主體可以交給 BibTeX ，所以文獻管理軟件應該處理的就是用戶所輸入的這些文獻，與 bib 文件的交互，但這些 bib 文件之間往往缺乏關聯，導致用戶使用時無比頭痛，需要一個軟件進行「中央化」管理。
文獻管理其實和筆記的管理有一定相似之處，於是在開發尺帛之餘，隨手寫了寸箋。
項目地址： https://github.com/821/BibInT