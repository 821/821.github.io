...
簡單來講，尺帛衹做這些事：轉換筆記、閱讀筆記、備份筆記、用指定編輯器打開筆記。因此，尺帛面向的是頻繁使用輕量級標記語言和文本編輯器來維護若干重要筆記的人，一般是比較 geek 向的。
Log:
2015/05/02 開工。
2015/05/06 投入使用。
2015/05/07 開源。
...
2015/05/18 功能：備份到 Dropbox ，免帳密，僅用 oauth2 access token 。
2015/05/27 功能： Tab ，開新 tab 時自帶 view 。代碼細節修改。
2015/05/28 功能：檢索當前文件。變更：取消獨立的 Edit List 按鈕，給 tabs 增加關掉按鈕。