...
三、所讀敎程太專業或重點不淸。
本文的目標是掛一漏萬，能半小時內讀完並對大部分內容有印象，而且這些內容「剛好」是最重要的。
...
常用模塊：
os 和 glob 處理文件與文件夾
requests, urllib/urllib.request 和 httplib/http.client 網頁訪問與下載
re 正則表達式
time 和 datetime 日期與時間
math 數學
threading 多線程
...
把本文迅速瀏覽一遍就可以寫了，寫自己想寫的。比如抓網站，或者按需生成文件，反正想幹嘛就幹嘛，別太複雜就行。
至於那些 tutorial 、習題，對非專業人員就是浪費腦細胞，有空再說。
使用時記住：反正 Python 是很強大的，無所不能的，遊戲 Civilization IV 和網盤 Dropbox 就是 Python 寫的，做不到說明自己水平不到而已。

h3. 搞淸楚版本

Python 有 2 和 3 ，官方力推 3 ，但是可能改動太大還是怎麼了，反正遇到了很大的阻力，所以現在二者並存。
基本上來講， 3 的命令格式更嚴格、規範一點， 2 比較鬆散。網上很多討論都是基於 2 的，要注意修改一下。但是 3 確實有些新功能。
...
凡是字符串都要加上引號，數字不加，所以 "123" 和 123 是不同的。
...
{% highlight python %}print(value, ..., sep=' ', end='\n', file=sys.stdout, flush=False){% endhighlight %}
...
變量的名稱除了那幾十個保留字隨便用。變量的類型有，有數字、字符串、列表、元組（衹讀的列表）、字典。可以這麼定義：
...
var=raw_input("Prompt") # 請用戶輸入變量，但卽使用戶輸是數字，其屬性仍是字符串，這個命令僅在 Python 2 可用
var=input("Prompt") # 也是用戶輸入，但輸入的是可以運算的，比如輸入 1+1 ，得到的不是 "1+1" 這個字符串，而是 2 ，但在 Python 3 當中，效果等同與 2 中的 raw_input{% endhighlight %}
