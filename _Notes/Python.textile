...
{% highlight python %}file object = open(file_name [, access_mode][, buffering]){% endhighlight %}
...
{% highlight python %}print "string"
print variable{% endhighlight %}
跟滿大街的其他語言一樣。
如果要輸出到一個文件，可以這麼寫：
{% highlight python linenos %}f=file("1.txt", "w")
print >>f,"string"
...
var=raw_input("Prompt") # 請用戶輸入變量，但卽使用戶輸是數字，其屬性仍是字符串
var=input("Prompt") # 也是用戶輸入，但輸入的是可以運算的，比如輸入 1+1 ，得到的不是 "1+1" 這個字符串，而是 2{% endhighlight %}
...
其實我個人更喜歡用 raw_input() 來終止程序，因爲窗口會保留。
