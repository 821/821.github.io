...
其實做個自己的腳本，一般用不上 IF ，人腦知道這個腳本不能做這事就排除了。不過， IF 結合 GOTO 可以變成循環，這倒是有點用：
<notextile>{% highlight bat linenos %}
@ECHO OFF
SET var=0
:continue
SET /A var+=1
...
就那麼幾個命令，不夠用，怎麼辦？
安裝 Cygwin ，使用 Linux 命令。有了這堆 Linux 命令，簡直什麼都有了。
微軟的 VBScript 或 PowerShell 也可增強。
